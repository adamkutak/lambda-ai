...
class APIFunction:
    ...
    def maybe_create_api_function(self) -> (int, str):
        ...
        result_code, message = self.validate_and_test_api_function(
            self.name, self.path, ai_response
        )
        print("----2-----")
        print(result_code)
        print(message)
    ...
    def validate_and_test_api_function(self, name, path, ai_response) -> (int, str):
        ...
        for test_case in self.test_cases:
            error_message = None
            e, response = api_server.query(path, test_case["input"])
...
class APIFile:
    ...
    def add_function(self, api_function: Dict[str, str]) -> str:
        ...
        assert api_function["path"] not in [func["path"] for func in self.functions]
