...
from lambda_ai.database.crud.api_container import create_api_environment, create_api_file, get_api_file, update_api_file
from lambda_ai.database.crud.api_function import create_api_function, get_api_function, update_api_function
from lambda_ai.database.crud.db import create_db, delete_db, get_all_dbs, get_db
from lambda_ai.database.main import SessionLocal
...
def test_create_and_retrieve_api_function():
    with db_test_session() as db:
        new_function = APIFunctionCreate(
            ...
            test_cases=[{"input": {"arg1": 5, "arg2": "test"}, "output": {"result": "test5"}}],
            force_use_db=False,
            is_async=False,
            attached_db=None,
        )
...
def test_update_api_function():
    with db_test_session() as db:
        ...
        # # Update the APIFunction
        # updated_function_data = APIFunctionCreate(
        #     name="UpdatedFunction",
        #     path="/updated_path",
        #     inputs={"arg1": "int", "arg2": "float"},
        #     outputs={"result": "float"},
        #     functionality="Updated Demonstration",
        #     test_cases=[{"input": {"arg1": 5, "arg2": 3.5}, "output": {"result": 8.5}}],
        #     force_use_db=True
        # )
        ...
        updated_function = update_api_function(db, created_function.id, **updated_function_data)
...
def test_add_db_to_api_function():
    with db_test_session() as session:
        ...
        updated_function = update_api_function(session, created_function.id, **updated_function_data)
...
def test_create_api_file_and_environment():
    with db_test_session() as session:
        ...
        current_functions = retrieved_api_file.functions.copy() # IMPORTANT: YOU MUST COPY THE DICT SO IT GETS A NEW MEM LOCATION

        current_functions["new_function"] = new_function

        updated_api_file = update_api_file(session, retrieved_api_file.id, functions=current_functions)
